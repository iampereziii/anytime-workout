...
describe("buildFactsBlock (rule 4 — every number is computed, none inferred)", () => {
  ...
  it("never renders a remaining-count line (ADR-0009 — layer retired)", () => {
    const block = buildFactsBlock(base);
    expect(block).not.toContain("Remaining today");
    expect(block).not.toContain("sets left");
  });

  it("never renders a day-status line — there is no done marker (ADR-0009 amendment)", () => {
    // "Finished for the day" is the coach's read of the recency facts, not an app
    // fact. If a day-status line ever reappears here, the marker has crept back in.
    const block = buildFactsBlock(base);
    expect(block).not.toContain("Day status");
    expect(block).not.toContain("TODAY DONE");
  });
...
describe("chatSystemPrompt (ADR-0007 coaching regime — no stored program)", () => {
  ...
  it("forbids reconciling logs against the suggestion (ADR-0009 — suggestion, not checklist)", () => {
    const p = chatSystemPrompt(block);
    expect(p).toContain("Today's suggestion is a suggestion, not a checklist");
    expect(p).toContain("never state or estimate how many sets are left");
  });
