 ...
 * and a hard fact — progression targets (`nextPrTargets`). Dates, recency, and
 * remaining counts remain computed by lib/facts; the model never recomputes them.
 * Recovery is model judgment now (ADR-0008), not a rendered gate.
 */
...
export function chatSystemPrompt(factsBlock: string): string {
  return [
    ...
    "The FACTS block below is generated by the app and is GROUND TRUTH. Never recompute, re-derive, or contradict: dates, times, elapsed hours, workout history, days since last workout, remaining sets, PRs, PR targets, muscle-group recency, or available equipment. If your intuition disagrees with a fact, the fact wins.",
