 ...
 * the deterministic signal it must not override is rendered here as GROUND TRUTH
 * and a hard fact — progression targets (`nextPrTargets`). Dates and recency remain
 * computed by lib/facts; the model never recomputes them. Recovery is model judgment
 * now (ADR-0008), not a rendered gate. Per ADR-0009 there is no remaining-count line
 * and no day-status line: the recommendation is a static suggestion, and whether the
 * owner is finished for the day is left to the coach's read of the recency facts.
 */
...
export interface FactsBlockInput {
  ...
  /** Current instant on the owner's clock, "yyyy-mm-dd HH:mm" app-tz (exact-time brief). */
  now?: string;
  days_since_last_workout: number | null; // null = no history (Flow 4)
  /** Hour-granular last-workout recency — live-logged sessions only; hours are
   *  computed by lib/facts, never by the model. Absent/null = date-only fallback
   *  (backdated entry, or rows predating timestamp capture). */
  last_workout?: { at: string; hours_since: number } | null;
  /** Computed sleep-ambiguity flag (lib/facts): day rolled AND gap < 16h — sleep is
   *  genuinely unknowable. Renders the explicit-unknown line the model must not fill in. */
...
export function buildFactsBlock(f: FactsBlockInput): string {
  ...
  if (f.now) lines.push(`Now (owner's clock): ${f.now}`);
  ...
  if (f.days_since_last_workout === null) {
    ...
    if (f.last_workout) {
      const h = f.last_workout.hours_since;
      const ago = h < 1 ? "less than an hour ago" : `~${h}h ago`;
      lines.push(`Last workout: ${f.last_workout.at} (${ago}, computed)`);
    }
    // Explicit unknown (never omitted when ambiguous — omission invites inference):
    // the day rolled but the gap is only hours; whether sleep happened is unknowable.
    if (f.sleep_state_ambiguous) {
      lines.push(
        "Slept since last workout: UNKNOWN — the calendar day has rolled but the last workout was only hours ago. The app cannot know whether the owner has slept. Never assume either way.",
      );
    }
 ...
 * today from history + recovery. Facts stay ground truth (ADR-0004); PR targets are
 * hard facts; today's card suggestion is ADVISORY — the model may disagree when the
 * facts support a better recommendation. ADR-0008: recovery is no longer a hard gate —
 * the coach decides rest vs. train from the muscle-recency facts + the RECOVERY RULES.
 */
export function chatSystemPrompt(factsBlock: string): string {
  return [
    ...
    "The FACTS block below is generated by the app and is GROUND TRUTH. Never recompute, re-derive, or contradict: dates, times, elapsed hours, workout history, days since last workout, PRs, PR targets, muscle-group recency, or available equipment. If your intuition disagrees with a fact, the fact wins.",
    ...
    "Today's suggestion is a suggestion, not a checklist. The app does not track how much of it the owner has completed, and neither do you: never state or estimate how many sets are left, and never reconcile their logged sets against the suggestion.",
    "The app does not know whether the owner is finished for the day — there is no 'done' flag. Read it from the recency facts and coach accordingly: if they have already trained recently (hours, not days), treat the day's work as done unless they say otherwise. Do not push today's card suggestion at someone who just trained, and never nag them to finish it.",
    "",
    "EXACT TIME & SLEEP (never assume):",
    "When the FACTS give hours since the last workout, reason in hours, not calendar days — a workout 8 hours ago is NOT \"yesterday's session\", and a workout early this morning may already be 15 hours behind you. The elapsed hours are computed by the app; never re-derive them from dates or clock times.",
    "If the FACTS include a 'Slept since last workout: UNKNOWN' line, the calendar day has rolled but only a few hours have passed. NEVER assume the owner has slept or that this is a fresh training day. If your answer depends on recovery, ask exactly ONE clarifying question first — e.g. \"Have you slept since that workout?\" — and prescribe only after the answer. Once the owner has answered in this conversation, respect that answer for the rest of it and do not ask again.",
    "",
    "COACHING PHILOSOPHY:",
    ...
    "If multiple workouts are viable, prioritize: (1) the muscle groups trained least recently, (2) the workout that best balances weekly volume.",
    "",
    "PROGRESSION:",
 ...
 * hard computed gate and PR targets are the progression anchor · v6 = exact-time
 * recency + sleep-state awareness: hour facts in the block; sleep-unknown means
 * compose conservative (workout-timing-sleep-state brief) · v7 = ADR-0008: the
 * deterministic recovery gate is retired; recovery is model judgment over the
 * muscle-recency facts + RECOVERY RULES (no RECOVERY REQUIRED line, no HARD gate).
 */
 ...
 * from the allowed catalog. PR targets arrive in the facts block as the one hard
 * signal it must respect; it never recomputes numbers the app already computed
 * (dates, recency, PR targets). Recovery is its own judgment now (ADR-0008): it may
 * return is_recovery = true off the muscle-recency facts, but nothing forces it.
 *
...
export function recommendationSystemPrompt(factsBlock: string, allowedExercises: readonly string[]): string {
  return [
    ...
    "",
    "GROUND TRUTH:",
    "The FACTS block below is computed by the application and is the source of truth. Never recompute or contradict any dates, day gaps, PR targets, or muscle-group recency. Every number you cite in the reason must come directly from the FACTS.",
    "",
    "EXACT TIME & SLEEP:",
    "When the FACTS give hours since the last workout, weigh them — elapsed hours are computed by the app; never re-derive them. If the FACTS include a 'Slept since last workout: UNKNOWN' line, the day has rolled but the last workout was only hours ago and you cannot ask the owner: compose the conservative option — lighter volume, non-overlapping muscle groups, or active recovery — never a full session that assumes overnight recovery happened.",
    "",
    "CHOOSING EXERCISES:",
...
export function parseSystemPrompt(): string {
  return [
    ...
    "- weight_lbs: a number when the user states one load for every set; null when none is stated or the movement is plain bodyweight. \"+10\" or \"BW+10\" means added load: 10.",
    '- Weight can change across sets (pyramid sets) — then weight_lbs is a per-set array, positionally paired with the reps. Parallel lists pair by position: "3 sets 12, 10, 8. 17.5lbs 20lbs 25lbs" means reps [12, 10, 8] with weight_lbs [17.5, 20, 25] (set 1 = 12 @ 17.5, set 2 = 10 @ 20, set 3 = 8 @ 25). Decimal weights are normal.',
    '- "25x12, 30x10, 35x8" (weight x reps) and "12@25, 10@30, 8@35" (reps @ weight) also mean per-set weights — return both arrays. For bodyweight movements a varying added load ("+10, +15, +20") is the array [10, 15, 20].',
    "- Never return a weight_lbs array when one load covers all sets — use the single number, exactly as before.",
